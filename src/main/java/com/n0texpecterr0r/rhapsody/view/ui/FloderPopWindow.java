...
public class FloderPopWindow extends PopupWindow {
    ...
    public FloderPopWindow(Activity context) {
        ...
        // 获取屏幕宽高
        popupWidth = size.x;
        popupHeight = size.y / 2 + 400;

        this.setContentView(contentView);   // 设置View
        this.setWidth(popupWidth);          // 设置弹出窗体的宽
        this.setHeight(popupHeight);        // 设置弹出窗体的高
        this.setFocusable(true);            // 设置弹出窗体可以点击
        this.setOutsideTouchable(true);     // 设置点击外部关闭
        this.setBackgroundDrawable(new ColorDrawable(WHITE));   // 设置全白背景
        this.update();                                          // 刷新
        this.setAnimationStyle(R.style.pop_anim_style);         // 设置动画
    ...
    public void show(View view) {
        ...
        showAtLocation(view, Gravity.NO_GRAVITY,
                (location[0]) - popupWidth / 2, location[1] - popupHeight);
